...
@click.pass_context
def cell_vhseg(ctx,title,tstop,outhoc,outappend):
    ...
    for mech in valid_mechs:
        ...
        with open(filename, 'w+') as f:
            ...
            code_blocks = ctg.mechanism_parse[mech].blocks
            for block in code_blocks:
                if block.__class__.__name__ == "Neuron":
                    ...
                    for act_var in act_vars:
                        act = [v for v in ctg.mechanism_dict[mech]["state_activation_vars"] if v['var'] == act_var['var']][0]
    ...
    for hoc_file in hoc_files:
        ...
        with open(hoc_file, 'r') as f:
            ...
            readon = False
            for line in lines:
                if "begintemplate " + template in line:
                    readon = True
                    found = True
                if readon:
                    template_text.append(line)
                if "endtemplate " + template in line:
                    readon = False
    if found:
