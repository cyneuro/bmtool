...
def cell_fir(ctx,title,min_pa,max_pa,increment,tstart,tdur):#, title, populations, group_by, save_file):
    ...
    click.echo("Using section " + colored.green(sec_split + "(0.5)") + " for injection and recording")
    ...
    fir_widget = FICurveWidget(template,i_increment=increment,i_start=min_pa,i_stop=max_pa,tstart=tstart,tdur=tdur,
        record_sec=sec_split, record_loc="0.5", inj_sec=sec_split, inj_loc="0.5")

    
    plot_widget = PlotWidget(tstop=ctg.tstop)
    plot_widget.add_expr(eval("fir_widget.passive_cell." + sec_split + "(0.5)._ref_v"),str(round(float(fir_widget.passive_amp),2)))
    for cell,amp in zip(fir_widget.cells, fir_widget.amps):
        plot_widget.add_expr(eval("cell." + sec_split + "(0.5)._ref_v"),str(round(float(amp),2)))
