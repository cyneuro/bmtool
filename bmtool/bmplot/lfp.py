...
def plot_spectrogram(
    ...
    f = f[f_idx]
    pcm = ax.pcolormesh(
        t, f, sxx[f_idx, :], shading="gouraud", vmin=vmin, vmax=vmax, rasterized=True
    )
...
def plot_population_spike_rates_with_lfp(
    ...
    is_trial_avg = (
        isinstance(time_range, list) and len(time_range) > 0 and isinstance(time_range[0], tuple)
    )
    ...
    for ax_idx, group in enumerate(plot_groups):
        ...
        if is_trial_avg and spike_rate_trials is not None:
            ...
            for pop in valid_pops:
                fr_mean, fr_sem = _compute_trial_average(
                    spike_rate_trials, pop, FR_type, target_length=target_length
                )
                (line,) = ax.plot(
                    plot_time, fr_mean, color=pop_color[pop], label=f"{pop} FR", linewidth=2
                )
                ax.fill_between(
                    plot_time, fr_mean - fr_sem, fr_mean + fr_sem, color=pop_color[pop], alpha=0.2
                )
        ...
        if is_trial_avg and power_trials is not None:
            ...
            for power_trial, label, color in zip(power_trials, freq_labels, freq_colors):
                power_mean, power_sem = _compute_trial_average_power(
                    power_trial, target_length=target_length
                )
                (line,) = ax2.plot(
                    plot_time, power_mean, color=color, label=label, linestyle="--", linewidth=2
                )
                ax2.fill_between(
                    plot_time,
...
def _extract_trials(
    spike_rate: Any, powers: List[Any], time_range: List[Tuple[float, float]]
...
def _compute_trial_average(
    spike_rate_trials: List[Any],
    pop_name: str,
    data_type: str = "smoothed",
    target_length: Optional[int] = None,
...
def _compute_trial_average_power(
    power_trials: List[np.ndarray], target_length: Optional[int] = None
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def plot_spike_rate_coherence(
    ...
    for i, grp_pair in enumerate(grp_pairs):
        ...
        if np.std(signal1) == 0 or np.std(signal2) == 0:
            ax.text(
                0.5, 0.5, "No variation in data", ha="center", va="center", transform=ax.transAxes
            )
        ...
        if not np.any(idx):
            ax.text(
                0.5, 0.5, "No valid coherence", ha="center", va="center", transform=ax.transAxes
            )
            ax.set_title(f"Coherence {pop1_name}-{pop2_name}")
            continue
        ...
        plt.sca(ax)
        fooof_results, fm = fit_fooof(
            f_filtered, cxy_filtered, **fooof_params, report=False, plot=True
        )
