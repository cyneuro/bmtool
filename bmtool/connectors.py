...
class CustomFilterConnector(UnidirectionConnector):
    """
    A connector that uses a custom filter function to determine connection probabilities.
    The filter function should return True/False, which then determines which probability
    (p_match or p_nonmatch) to use for the connection.

    Parameters:
        filter_fn: Function that takes (source_node, target_node) and returns bool
        p_match: Connection probability when filter returns True
        p_nonmatch: Connection probability when filter returns False
        p_arg: Optional argument for probability calculations (e.g. distance)
        n_syn: Number of synapses per connection
    """
    ...
    def __init__(
        ...
        super().__init__(
            p=1.0,  # Will be overridden in make_connection
            p_arg=p_arg,
            n_syn=n_syn,
            verbose=verbose,
        ...
        self.p_match = self.constant_function(p_match) if not callable(p_match) else p_match
        self.p_nonmatch = (
            self.constant_function(p_nonmatch) if not callable(p_nonmatch) else p_nonmatch
        )

        # Statistics
        self.n_matching = 0
        self.n_matching_connected = 0
        self.n_nonmatching = 0
        self.n_nonmatching_connected = 0

    def make_connection(self, source, target, *args, **kwargs):
        """Makes connections based on filter function result"""
        ...
        if self.iter_count == 0:
            ...
            if self.verbose:
                src_str, trg_str = self.get_nodes_info()
                print(
                    f"\nStart building filtered connections\n  from {src_str}\n  to {trg_str}",
                    flush=True,
                )

        # Check filter
        is_match = self.filter_fn(source, target)

        # Get appropriate probability
        p_arg = self.p_arg(source, target) if self.p_arg else None
        p = self.p_match(p_arg) if is_match else self.p_nonmatch(p_arg)
        ...
        if is_match:
            ...
            self.n_nonmatching += 1

        # Make connection
        if possible and np.random.random() < p:
            nsyns = self.n_syn(source, target) if callable(self.n_syn) else self.n_syn
            self.add_conn_prop(source.node_id, target.node_id, p_arg)
            self.n_conn += 1

            # Update connection statistics
            if is_match:
                self.n_matching_connected += 1
            else:
                self.n_nonmatching_connected += 1
        else:
            nsyns = 0
        ...
        if self.iter_count == self.n_pair:
            if self.verbose:
                ...
                print("\nFilter statistics:")
                print(
                    f"  Matching pairs: {self.n_matching_connected}/{self.n_matching} connected "
                    + f"({100.0*self.n_matching_connected/max(1,self.n_matching):.1f}%)"
                )
                print(
                    f"  Non-matching pairs: {self.n_nonmatching_connected}/{self.n_nonmatching} connected "
                    + f"({100.0*self.n_nonmatching_connected/max(1,self.n_nonmatching):.1f}%)"
                )
                self.timer.report("Done! \nTime for building connections")
            if self.save_report:
                self.save_connection_report()
