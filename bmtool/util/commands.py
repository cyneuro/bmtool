...
def cell_tune(
    ...
    if not title:
        title = (
            template
            + " - Cell Configurator - Interface generated by BMTool (https://github.com/cyneuro/bmtool)"
        )
...
def cell_fir(
    ...
    if not title:
        title = (
            template
            + " - Cell FI Curve - Interface generated by BMTool (https://github.com/cyneuro/bmtool)"
        )
        # ctg.set_title(title)
...
def cell_vhsegbuild(
    ...
    if not title:
        title = (
            template
            + " - V1/2 Segregation - Interface generated by BMTool (https://github.com/cyneuro/bmtool)"
        )
    # import pdb;pdb.set_trace()
...
@cell.command("passive", help="Calculate passive properties for a given cell template")
@click.option("--inj-amp", type=click.FLOAT, default=-100.0, help="Injection amplitude (pA)")
@click.option("--inj-delay", type=click.FLOAT, default=200.0, help="Injection delay (ms)")
@click.option("--inj-dur", type=click.FLOAT, default=1000.0, help="Injection duration (ms)")
@click.option("--tstop", type=click.FLOAT, default=1200.0, help="Total simulation time (ms)")
@click.option(
    "--method",
    type=click.Choice(["simple", "exp", "exp2"]),
    default="simple",
    help="Method to estimate membrane time constant",
)
@click.option("--plot", is_flag=True, help="Display the membrane potential plot")
@click.pass_context
def cell_passive(ctx, inj_amp, inj_delay, inj_dur, tstop, method, plot):
    ...
    profiler = Profiler(template_dir=hoc_folder, mechanism_dir=mod_folder)
    if not template:
        template = questionary.select("Select a cell:", choices=profiler.hoc_templates).ask()

    profiler.passive_properties(
        template,
        inj_amp=inj_amp,
        inj_delay=inj_delay,
        inj_dur=inj_dur,
...
@cell.command("zap", help="Calculate impedance amplitude profile (ZAP) for a given cell template")
@click.option("--inj-amp", type=click.FLOAT, default=100.0, help="Chirp amplitude (pA)")
@click.option("--fstart", type=click.FLOAT, default=0.0, help="Start frequency (Hz)")
@click.option("--fend", type=click.FLOAT, default=100.0, help="End frequency (Hz)")
@click.option("--inj-delay", type=click.FLOAT, default=200.0, help="Injection delay (ms)")
@click.option("--inj-dur", type=click.FLOAT, default=1000.0, help="Chirp duration (ms)")
@click.option("--tstop", type=click.FLOAT, default=1200.0, help="Total simulation time (ms)")
@click.option("--plot", is_flag=True, help="Display the ZAP response and impedance plots")
@click.pass_context
def cell_zap(ctx, inj_amp, fstart, fend, inj_delay, inj_dur, tstop, plot):
    from bmtool.singlecell import Profiler
    ...
    profiler.impedance_amplitude_profile(
        template,
        inj_amp=inj_amp,
        fstart=fstart,
        fend=fend,
...
def cell_vhseg(
    ...
    if not title:
        title = (
            template + " - V1/2 - Interface generated by BMTool (https://github.com/cyneuro/bmtool)"
        )
    # import pdb;pdb.set_trace()
    sec = ctg.root_sec.hname()
    sec_split = sec.split(".")[-1]
